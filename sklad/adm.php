                    ...
                    <tr>
                      <td align="right">
						<div align="justify" class="text">
											  ...
											  <p align="center" class="стиль24">Адміністрування</p>
<?php if ($dostup != 1) { ?>
...
if($kart_id = $_GET['kart'])
{
	$result=pg_query($connection, "select * from cartlist where id = $kart_id;");
	$db=pg_fetch_array($result);
	
}

if($print_id = $_GET['printr'])
{
	$result=pg_query($connection, "select * from printlist where id = $print_id;");
	$db=pg_fetch_array($result);
...
if($kart = $_POST['kart']) 
{
	$result=pg_query($connection, "select * from cartlist where modelcart = '$kart';");
	if(pg_num_rows($result) > 0)
	{
	
		if(isset($_POST['del4']))
		{
			if (pg_query($connection, "delete from cartlist where modelcart = '$kart';")) echo "Картридж $kart видалено!"; 
		}
	 	else
		{
			if (pg_query($connection, "update cartlist set nomenklnumber = '$_POST[NomenklNumber]', resurs = $_POST[Resurs] where modelcart = '$kart';")) echo "Картридж $kart змінено!"; else  echo "Картридж $kart не вдалося змінити!"; 
			$result=pg_query($connection, "select * from cartlist where modelcart = '$kart';");
			$db=pg_fetch_array($result);
		}
	}
	...
	{
		if($kart != '') if (pg_query($connection, "insert into cartlist (modelcart, nomenklnumber, resurs) values ('$kart', '$_POST[NomenklNumber]', $_POST[Resurs]);")) echo "Картридж $kart добавлено"; else echo "Картридж $kart - $_POST[NomenklNumber] - $_POST[Resurs] не вдалося додати";
	}		
...
if($printr = $_POST['printr']) 
{
	$result=pg_query($connection, "select * from printlist where modelprinter = '$printr';");
	if(pg_num_rows($result) > 0)
	{
	
		if(isset($_POST['del5']))
		{
			if (pg_query($connection, "delete from printlist where modelprinter = '$printr';")) echo "Принтер $printr видалено!"; 
		} 
		else 
		{
			if (pg_query($connection, "update printlist set coderesurs = '$_POST[kod]', tonerb = $_POST[tonerb], tonerc = $_POST[tonerc], tonerm = $_POST[tonerm], tonery = $_POST[tonery], tonercl = $_POST[tonercl], drumb = $_POST[drumb], drumc = $_POST[drumc], drumm = $_POST[drumm], drumy = $_POST[drumy] where modelprinter = '$printr' ;")) echo "Принтер $printr змінено!"; else echo "Принтер $printr не вдалося змінити!"; 
			$result=pg_query($connection, "select * from printlist where modelprinter = '$printr';");
			$db=pg_fetch_array($result);
		}	 
	}
	...
	{
		if($printr != '') if (pg_query($connection, "insert into printlist (modelprinter, coderesurs, tonerb, tonerc, tonerm, tonery, tonercl, drumb, drumc, drumm, drumy) values ('$_POST[printr]', '$_POST[kod]', $_POST[tonerb], $_POST[tonerc], $_POST[tonerm], $_POST[tonery], $_POST[tonercl], $_POST[drumb], $_POST[drumc], $_POST[drumm], $_POST[drumy] );")) echo "Принтер $printr добавлено"; else echo "Принтер $printr не вдалося добавити"; 
		echo "<br>$_POST[printr], $_POST[kod], $_POST[tonerb], $_POST[tonerc], $_POST[tonerm], $_POST[tonery], $_POST[tonercl], $_POST[drumb], $_POST[drumc], $_POST[drumm], $_POST[drumy] ";
	}		
											  ...
											  <table width="100%" border="1" cellspacing="0" cellpadding="0">
                                                <tr>
                                                  <td><form name="form1" method="post" action="">
                                                    <table  width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
                                                      <tr>
                                                        <td colspan="2" bgcolor="#66CCCC"><div align="center"><strong>Робітник
                                                          <select name="rabotnik" id="rabotnik" onChange="top.location.href = 'adm.php?rabotnik='+this.options[this.selectedIndex].value;">
                                                            <?php

$result2=pg_query($connection, "select * from rabotniki order by rabotniki_id");
echo "<option value=''>Виберіть робітника для редагування</option>";
...
?>
                                                      ...
                                                      <tr>
                                                        <td>Призвіще
                                                          <input name="rab" type="text" id="rab" <?php echo "value=\"$rabotnik\"" ?>></td>
                                                        <td>Логін
                                                          <input name="login" type="text" id="login" <?php echo "value=\"$log\"" ?>></td>
                                                      </tr>
                                                      <tr>
                                                        <td>Пароль
                                                        <input name="passw" type="text" id="passw" value=""></td>
                                                        <td>Адміністратор
                                                          <input name="dostup" type="checkbox" id="dostup" value="1" <?php if($dostup == 1) echo "checked"; ?>></td>
                                                      </tr>
                                                      <tr>
                                                        <td>Видалити
...
?>
                                                    <form name="form1" method="post" action="">
                                                      ...
                                                      <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                        <tr>
                                                          <td colspan="2" bgcolor="#66CCCC"><div align="center"><strong>Обладнання
                                                            <select name="tipy" id="tipy" onChange="top.location.href = 'adm.php?tip='+this.options[this.selectedIndex].value;">
                                                              <?php
...
?>
                                                ...
                                                <tr>
                                                  <td><form name="form1" method="post" action="">
                                                    ...
                                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                      <tr>
                                                        <td colspan="2" bgcolor="#66CCCC"><div align="center"><strong>Стан
...
?>
											  ...
											  <form name="kartr" method="post" action="">
											  <p align="center">
                                              <table width="100%" border="0" cellspacing="0" cellpadding="0">
											    <tr>
											      <td colspan="3" bgcolor="#66CCCC"><div align="center"><strong>Картриджі 
											            <select name="kartr" id="kartr" onChange="top.location.href = 'adm.php?kart='+this.options[this.selectedIndex].value;">
											          <?php

$result2=pg_query($connection, "select * from CartList order by modelcart");
echo "<option value=''>Виберіть картридж для редагування</option>";
while($db2=pg_fetch_array($result2)):
 $nspi=$db2[id];
 $spi=$db2[modelcart];
if($nspi == $kart_id) 
...
?>
											    ...
											    <tr>
											      <td >Модель картриджу
											        <input name="kart" type="text" id="kart" <?php echo "value=\"$db[modelcart]\"" ?>></td> <td>Номенкл.№
                                                          <input type="text" name="NomenklNumber" id="NomenklNumber" <?php echo "value=\"$db[nomenklnumber]\"" ?>></td>
                                                          <td>Ресурс
                                                          <input name="Resurs" type="text" id="Resurs" value="0" <?php echo "value=\"$db[resurs]\"" ?>></td>
										        </tr>
											    <tr>
											      <td colspan="2">Видалити
											        <input name="del4" type="checkbox" id="del4" value="1"></td>
											      <td><input type="submit" name="Submit5" value="Подтвердить"></td>
										        </tr>
										      </table>
...
<form name="print" method="post" action="">
                                              ...
                                              <table width="100%" border="0" cellspacing="0" cellpadding="0">
											    <tr>
											      <td colspan="3" bgcolor="#66CCCC"><div align="center"><strong>Принтери
											            <select name="printr" id="printr" onChange="top.location.href = 'adm.php?printr='+this.options[this.selectedIndex].value;">
											          <?php
...
?>
											    ...
											    <tr>
											      <td colspan="2">Модель принтеру
											        <input name="printr" type="text" id="printr" <?php echo "value=\"$db[modelprinter]\"" ?>></td>
											      <td >Код
											        <input type="text" name="kod" id="kod" <?php echo "value=\"$db[coderesurs]\"" ?>></td>
										        </tr>
											    <tr >
											      <td colspan="3" bgcolor="#CCCCCC" >Toner &quot;B&quot;

<select name="tonerb" id="tonerb">
<?php
$result2=pg_query($connection, "select * from CartList");
...
?>
											      ...
											      <td colspan="3"  bgcolor="#99FFFF">Toner &quot;C&quot;
											        <select name="tonerc" id="tonerc">
											          <?php
$result2=pg_query($connection, "select * from CartList");
echo "<option value=0></option>";
while($db2=pg_fetch_array($result2)):
...
?>
											      ...
											      <td  colspan="3" bgcolor="#FF99FF">Toner &quot;M&quot;
											        <select name="tonerm" id="tonerm">
											          <?php
$result2=pg_query($connection, "select * from CartList");
echo "<option value=0></option>";
while($db2=pg_fetch_array($result2)):
...
?>
											    ...
											    <tr>
											      <td  colspan="3" bgcolor="#FFFF99" >Toner &quot;Y&quot;
											        <select name="tonery" id="tonery">
											          <?php
$result2=pg_query($connection, "select * from CartList");
echo "<option value=0></option>";
while($db2=pg_fetch_array($result2)):
...
?>
											      ...
											      <td colspan="3"  bgcolor="#FFCC99">Toner &quot;CL&quot;
											        <select name="tonercl" id="tonercl">
											          <?php
$result2=pg_query($connection, "select * from CartList");
echo "<option value=0></option>";
while($db2=pg_fetch_array($result2)):
...
?>
											      ...
											      <td  colspan="3" bgcolor="#CCCCCC">Drum &quot;B&quot;
											        <select name="drumb" id="drumb">
											          <?php
$result2=pg_query($connection, "select * from CartList");
echo "<option value=0></option>";
while($db2=pg_fetch_array($result2)):
...
?>
											    ...
											    <tr>
											      <td colspan="3"  bgcolor="#99FFFF" >Drum&quot;C&quot;
											        <select name="drumc" id="drumc">
											          <?php
$result2=pg_query($connection, "select * from CartList");
echo "<option value=0></option>";
while($db2=pg_fetch_array($result2)):
...
?>
											      ...
											      <td  colspan="3" bgcolor="#FF99FF">Drum&quot;M&quot;
											        <select name="drumm" id="drumm">
											          <?php
$result2=pg_query($connection, "select * from CartList");
echo "<option value=0></option>";
while($db2=pg_fetch_array($result2)):
...
?>
											      ...
											      <td  colspan="3" bgcolor="#FFFF99">Drum &quot;Y&quot;
											        <select name="drumy" id="drumy">
											          <?php
$result2=pg_query($connection, "select * from CartList");
echo "<option value=0></option>";
while($db2=pg_fetch_array($result2)):
...
?>
											    ...
											    <tr>
											      <td colspan="2">Видалити
											        <input name="del5" type="checkbox" id="del5" value="1"></td>
											      <td><input type="submit" name="Submit5" value="Подтвердить"></td>
										        </tr>
										      </table>
